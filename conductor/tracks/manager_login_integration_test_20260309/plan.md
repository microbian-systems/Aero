...
- [x] **Task: Create `test/Aero.Cms.Manager.IntegrationTests` project** [f0fe306]
    - [x] Create `.csproj` with references to `MvcWeb`, `Alba`, `xunit`, `Marten.TestDriver`.
- [x] **Task: Set up `AlbaHost` for Integration Testing** [7df246a]
    ...
    - [x] Configure the host to use a test RavenDB store (Marten.TestDriver) and ensure `IdentityWithSeed` is called.
