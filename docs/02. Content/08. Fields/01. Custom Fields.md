...
> This article covers how to implement custom fields for Aero. If you want to read more about the standard field included, please refer to [Fields](./).

Fields are the lowest level of content in Aero CMS and are used to build up **Regions** and **Blocks**. In many cases the fields included in Aero are sufficient, but you might also want to create custom fields for you application. One use case is for example if you would like to reference a custom entity in your application from a Page or Post, much like the included `PageField` does.
...
In order to create a custom field your class has to implement the `IField` interface from the `Aero.Extend` namespace. The interface contains the following methods that needs to be implemented:
...
All available fields has to be registered in the singleton `Aero.App` **after** the app has been initialized.

~~~ csharp
Aero.App.Init(api);

// Register custom fields
Aero.App.Fields.Register<SimpleStringField>();
~~~
