...
By default Aero allows the following files to be uploaded into the **Media** library.
...
If you need to handle more media types in your application you can easily register them in your application startup after the `Aero.App` has been initialized. The following code will for example allow `.gif` images.

~~~ csharp
Aero.App.MediaTypes.Images.Add(".bmp", "image/bmp");
~~~

It's very import to classify the media types your add correctly in terms of what type it is (document, image or video). This is so that Aero knows what to do with the file.
...
~~~ csharp
Aero.App.MediaTypes.Images.Add(".svg", "image/svg+xml", false);
~~~
...
~~~ csharp
Aero.App.MediaTypes.MetaProperties.Add("ExternalId");
~~~
