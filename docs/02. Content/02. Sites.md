...
When you start your Aero application a site is automatically created for you with the name **Default Site** if there are no sites available. This site is only created on startup if there are no sites available, which means that after it has been created you can change the name to something different if desired.

Many calls to the Aero `Api` accept a Site Id as an optional parameter. If this parameter is omitted the default site will be used. Please note that if you have more than one site in your application you can choose which one should be the default site.
