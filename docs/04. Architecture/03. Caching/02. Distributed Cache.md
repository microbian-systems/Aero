...
The Distributed Cache is dependent on the standard `IDistributedCache`. The distributed cache is included in the core `Aero` package meaning that no additional packages needs to be installed.
...
public void ConfigureServices(IServiceCollection services)
{
    ...
    services.AddAeroDistributedCache();
