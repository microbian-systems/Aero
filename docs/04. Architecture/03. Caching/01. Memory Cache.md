...
The Memory Cache is an **in-process** memory cache dependent on the standard `IMemoryCache`. The memory cache is included in the core `Aero` package meaning that no additional packages needs to be installed.
