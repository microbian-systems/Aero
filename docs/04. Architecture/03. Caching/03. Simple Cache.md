...
The Simple Cache is an **in-process** memory cache based on an `IDictionary<string, object>`. It's fast and does the job for simple runtime scenarios. The simple cache is included in the core `Aero` package meaning that no additional packages needs to be installed to use it.
