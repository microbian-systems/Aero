...
> Please remember that if you are going to use our provided packages for ASP.NET Identity you will **also need to include** the MySQL package for Identity. For more information, please refer to the article about using [Identity](../authentication/identity) for Aero.

## Installation

To use MySQL you need to add the NuGet package for running Aero with `Entity Framework Core` and `MySQL`.

~~~ bash
PM> install-package Aero.Data.EF.MySql
~~~

#### Optional

~~~ bash
PM> install-package Aero.AspNetCore.Identity.MySQL
~~~
...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        services.AddAero(options =>
        {
            ...
            options.UseEF<Aero.Data.EF.MySql.MySqlDb>(o =>
            {
                o.UseMySql(connString);
            });

            // Optional
            // First add nuget package Aero.AspNetCore.Identity.MySQL
            options.UseIdentityWithSeed<Aero.AspNetCore.Identity.MySQL.IdentityMySQLDb>(db => db.UseMySql(connString));
...
As of Aero Version `10.0`, the MySQL package introduces a breaking change on how the database is configured. In addition to the connection string, the used database version needs to be specified. This can be accomplished by setting the specific version, for exmple
...
That's it! You're now using MySQL with Aero.
