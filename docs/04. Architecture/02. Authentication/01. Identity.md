...
~~~ csharp
> dotnet add package Aero.AspNetCore.Identity.SQLite
~~~

##### SQLServer

~~~ csharp
> dotnet add package Aero.AspNetCore.Identity.SQLServer
~~~

##### MySQL

~~~ csharp
> dotnet add package Aero.AspNetCore.Identity.MySQL
~~~

## Registering The Service

You register Identity Security in `ConfigureServices()`. There are two methods available for adding the services, `AddAeroIdentity` and `AddAeroIdentityWithSeed`. The later of them seeds the default admin user with username admin and password password. How you register the service is also dependent on which database provider you use:
