...
To create a serializer you must implement the `ISerializer` interface from the `Aero.Extend` namespace.
...
You register your custom serializers in the application startup. The core serializers are registered when calling `Aero.App.Init()`. This is how the ImageSerializer is registered.

~~~ csharp
Aero.App.Serializers.Register<ImageField>(new ImageFieldSerializer());
~~~

This basically lets Aero know that when an `ImageField` is loaded from, or saved to the database, the specified serializer should be used. As you can see, all serializers are singletons and should not contain any state or instance information.