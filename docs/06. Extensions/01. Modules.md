...
If you want to distribute a set of custom components, a good way to do it is to package them into a **Module**. In Aero a module is really nothing more than a NuGet-package (or project if you're creating a module for your self) that supplies some extensions methods and registers a class implementing the `Aero.Extend.IModule` interface.

If you have installed our project templates you can easily create a new module with the command `dotnet new Aero.module`. You can find the source code for the module template [here](https://github.com/AeroCMS/Aero.core.templates/tree/master/src/module).
...
When `App.Init()` is called from Configure in `Startup.cs` all registered modules are initialized. If your code is dependent on other parts of the application being initialized, packaging it as a module **ensures** that your code is executed at the correct time, and not before the main Aero App is ready.
...
In order to be registered in the Aero App, all modules need to have a class that implement the `Aero.Extend.IModule` interface.
...
namespace SimpleModule
{
    ...
    public class Module : IModule
    {
        ...
        /// Gets the Version
        /// </summary>
        public string Version => Aero.Utils
        ...
        /// Gets the icon url.
        /// </summary>
        public string IconUrl => "http://Aerocms.org/assets/twitter-shield.png";
