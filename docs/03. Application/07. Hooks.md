...
Hooks is a simple way to change, or extend the default behaviour of Aero CMS. Hooks can either **extend** the default behaviour or **replace** a behaviour depending on the hook. Hooks should be added in the `Configure` method in your application startup **after** `Aero.App` has been initialized.
...
This hook **extends** the built in Sitemap generation and can be used for adding custom entries, when you for example have pages in your site that is not controlled by Aero CMS.
...
The following hooks are available when accessing pages through the `PageService`. Please note that all models are passed in as `Aero.Models.PageBase` and must be casted to the applicable type.
...
The following hooks are available when accessing posts through the `PostService`. Please note that all models are passed in as `Aero.Models.PostBase` and must be casted to the applicable type.
...
The following hooks are available when accessing site content through the `SiteService`. Please note that all models are passed in as `Aero.Models.SiteContentBase` and must be casted to the applicable type.
