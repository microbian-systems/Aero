...
> The Application Service is part of the package `Aero.AspNetCore` and is intended to be used when building integrated web applications with `ASP.NET`.
...
The Application Service is needed by all middleware components in Aero CMS is registered automatically when calling `UseCms` in `ConfigureServices`.
