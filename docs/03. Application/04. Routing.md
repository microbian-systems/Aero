...
> This article cover how the basic routing mechanism of Aero works. For more advanced scenarios, like sending custom route parameters to page instances, please refer to [Advanced Routing](advanced-routing).

There's no magical tricks when it comes to the routing of Aero CMS. Aero relies **100%** on the underlying web framework of your choice to handle the requests in the end, may it be MVC **or** Razor Pages. In the follwing examples we will asume that we have two **Page Types** defined called `BasicPage` and `AdvancedPage`, and that they have the following routes set up.
...
As you can see, the Page Type `BasicPage` does not have a route explicitly specified. When there's no route specified Aero will rewrite the request to the **default route** of the Content Type. The following routes are default for the different core types:
...
Here's a simple description of what happens when a request comes to a Aero application.
...
When a request is handled by the middleware the query string parameter `Aero_handled=true` is added to the rewritten URL. This is done to tell **other Aero middleware** later in the pipeline that the request has been handled so there's no unnecessary processing done.
...
`GET /advanced?id=...&startpage=true&Aero_handled=true`
...
`GET /page?id=...&startpage=true&Aero_handled=true`
