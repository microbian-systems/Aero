...
The configuration in Aero can be updated both from the manager interface and from code. To set the configuration in your `Startup` class just inject `Aero.Config` into your `Configure` method.

~~~csharp
public void Configure(..., Aero.Config config)
{
    ...
}
~~~
